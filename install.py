...
if sys.version_info[0] < 3 or sys.version_info[0] == 3 \
    and (sys.version_info[1] < 8 or sys.version_info[1] > 10):
    raise Exception('Python ' + sys.version_info[0] + '.'
                    + sys.version_info[1]
...
conda_command = ['conda', 'install', '-c', 'nvidia/label/cuda-11.X.8',
                 'cuda-nvcc']
...
if osused == 'Darwin':
    ...
    if something_wrong_with_nvcc == True and flag_install_pythorch_cpu \
        == False:
        ans = \
            input('Something is wrong with NVCC. Do you want to install the CPU version of pythorch? [Y/n]'
                  )
        if ans.strip() == 'Y':
            flag_install_pythorch_cpu = True
        else:
...
if osused == 'Linux':
    ...
    if rc != 0:
        ...
        try:
            check_call(['sudo', 'apt-get', 'install', '-y',
                       'libgdal-dev'], stdout=open(os.devnull, 'wb'),
                       stderr=STDOUT)
        except:
            raise Exception('Impossible to install libgdal-dev. Please install manually libgdal-dev before running this script.\nInstallation aborted.'
                            )
        result = subprocess.getstatusoutput('gdal-config --version')
        output = result[1]
        rc = result[0]
    ...
    try:
        check_call(['sudo', 'apt-get', 'install', '-y',
                   'libxcb-xinerama0'], stdout=open(os.devnull, 'wb'),
                   stderr=STDOUT)
    except:
    ...
    if rc != 0:
        ...
        try:
            ...
            raise Exception('Impossible to install gdal through homebrew. Please install manually gdal before running this script.\nInstallation aborted.'
                            )
        result = subprocess.getstatusoutput('gdal-config --version')
        output = result[1]
        rc = result[0]
...
if osused != 'Windows':
    try:
        ...
        if out[0] != 0:
            if osused == 'Darwin':
                ...
                try:
                    ...
                    raise Exception('Impossible to install cmake through homebrew. Please install manually cmake before running this script.\nInstallation aborted.'
                                    )
            elif osused == 'Linux':
                print 'Trying to install cmake...'
                from subprocess import STDOUT, check_call
                ...
                try:
                    check_call(['sudo', 'apt-get', 'install', '-y',
                               'cmake'], stdout=open(os.devnull, 'wb'),
                               stderr=STDOUT)
                except:
                    raise Exception('Impossible to install cmake. Please install manually cmake before running this script.\nInstallation aborted.'
                                    )
        os.chdir('coraline')
        ...
        raise Exception('Cmake not found. Coraline library cannot be compiled. Please install cmake first.\nInstallation aborted.'
                        )
...
if osused != 'Windows':
    ...
    filename_gdal = 'gdal-' + gdal_win_version + '-cp' + pythonversion \
        + '-cp' + pythonversion
    filename_rasterio = 'rasterio-' + rasterio_win_version + '-cp' \
        + pythonversion + '-cp' + pythonversion
    ...
    if rasterio_is_installed is not None:
        ...
        print ('RASTERIO ', rasterio.__version__,
               ' is installed. Version ', rasterio_win_version,
               ' is required.')
    else:
        ...
        try:
            ...
            urllib.request.urlretrieve(base_url_rasterio,
                    this_directory + '/' + filename_rasterio)
        except:
    ...
    if gdal_is_installed is not None:
        ...
        print ('GDAL ', osgeo.gdal.__version__,
               ' is installed. Version ', gdal_win_version,
               ' is required.')
    else:
        ...
        try:
            ...
            opener.addheaders = [('User-agent', 'Mozilla/5.0')]
            urllib.request.install_opener(opener)
            urllib.request.urlretrieve(base_url_gdal, this_directory
                    + '/' + filename_gdal)
        except:
            raise Exception('Cannot download ' + slib + '.')
